...
def get_entropy():
    fl_dir = f"fl_scores/score_{fl_tool}"
    results_dir = f"fl_entropy_results/results_{fl_tool}_{model_name}"
    ...
    for s in samples:
        ...
        sus_counter = 0
        for sl in sus_lines:
            if sl.line_num in bug_line_list:
                is_bug = True
            else:
                is_bug = False
            ...
            outputs = infiller.generate(gen_prompt_toks, num_return_sequences=1)
